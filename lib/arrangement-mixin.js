  ...
  this.Drb_arrangement = (clasz = Object) => {
    return class extends clasz {
      ...
      _shape_hyphenated(cfg) {
        ...
        var I, L, R, V, ad, adi_0, ads, alt, alt_max, doc, dx0, fontnick, i, len, new_alt, par, ref, schema, sg_ads, shy, shy_row, text;
        ({fontnick, doc, par, ads, alt} = cfg);
        ({schema} = this.cfg);
        ({V, I, L} = this.sql);
...
from ${schema}.ads where ( doc = $doc ) and ( par = $par );`, {doc, par}));
        new_alt = alt_max;
        ref = this.db.all_rows(SQL`select adi, sgi from ${schema}.ads
  where true
    and ( doc = $doc )
    and ( par = $par )
    and ( br  = 'shy' )
    and ( alt = $alt );`, {doc, par, alt});
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          shy_row = ref[i];
          sg_ads = this.db.all_rows(SQL`select * from ads
  where true
    and ( doc = $doc )
    and ( par = $par )
    and ( sgi = $sgi )
  ...
  order by doc, par, adi;`, {
          ...
          text = ((function() {
            var j, len1, results;
            results = [];
            for (j = 0, len1 = sg_ads.length; j < len1; j++) {
              ad = sg_ads[j];
              results.push(ad.adi === shy_row.adi ? '- ' : ad.chrs);
            }
            return results;
          })()).join('');
          adi_0 = sg_ads[0].adi;
      ...
      //---------------------------------------------------------------------------------------------------------
      _shape_text(cfg) {
        var ad, adi, adi_0/* NOTE optional first AD index */, adi_0_given, ads, alt, ced_x, ced_y, doc, dx0/* NOTE optional x reference coordinate */, ed_x, font_idx, fontnick, i, idx, last_ad, last_adi, len, missing, par, sgi, text, this_adi, width;
        ({fontnick, text, adi_0, dx0, doc, par, alt, sgi} = cfg);
