  ...
  this.Drb_outlines = (clasz = Object) => {
    return class extends clasz {
      ...
      _shape_hyphenated(cfg) {
        var R, ad, ads, doc, fm, fontnick, hyphen_ad, i, len, par, shy_ad, shy_adi, shy_adi_1, shy_adi_2, shy_segments, vrt;
        ({fontnick, ads, shy_segments, doc, par, fm, vrt} = cfg);
        R = [];
        /* TAINT use proper validation */
        debug('^3493073^', fm);
        if (fm == null) {
          throw new Error("^8490353^ REPLACE WITH PROPER ERROR need fontmetrics");
        }
//.......................................................................................................
        for (i = 0, len = shy_segments.length; i < len; i++) {
          ({shy_adi, shy_adi_1, shy_adi_2} = shy_segments[i]);
          debug('^5006^', {shy_adi, shy_adi_1, shy_adi_2});
          urge('^5006^', (function() {
            var j, len1, ref, results;
            ref = ads.slice(shy_adi_1, shy_adi);
            results = [];
          ...
          if (shy_adi < shy_adi_2) {
            urge('^5006^', (function() {
              var j, len1, ref, results;
              ref = ads.slice(shy_adi + 1, +shy_adi_2 + 1 || 9e9);
              results = [];
              for (j = 0, len1 = ref.length; j < len1; j++) {
                ad = ref[j];
                results.push(ad.chrs);
          ...
          ads.push(hyphen_ad);
          if ((shy_adi_1 === shy_adi && shy_adi === shy_adi_2)) {
            null;
          }
        }
        //.......................................................................................................
      ...
      _shape_text(cfg) {
        /* TAINT will not properly handle multiple SHYs in the same segment (this might happen in ligatures
           like `ffi`) */
        var ad, adi, ads, ced_x, ced_y, doc, ed_x, font_idx, fontnick, i, last_ad, last_adi, len, missing, par, sgi, shy_adi, shy_adi_1, shy_segments, text, this_adi, vrt, width;
        ({fontnick, text, doc, par, vrt} = cfg);
      ...
      get_font_metrics(cfg) {
        var R, font_idx, fontnick, hyphen_ad;
      ...
      * insert_and_walk_outlines(cfg) {
      ...
      compose(cfg) {
        ...
        this.db(() => {
          ...
          var ad, base, i, insert_ad, len, row, vnr;
          insert_ad = this.db.prepare((base = this.sql).insert_ad != null ? base.insert_ad : base.insert_ad = this.db.create_insert({
