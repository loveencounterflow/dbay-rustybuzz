(function() {
  ...
  this.Drb_outlines = (clasz = Object) => {
    return class extends clasz {
      ...
      //-----------------------------------------------------------------------------------------------------------
      shape_text(cfg) {
        var R, bytes, d, font_idx, fontnick, i, idx, len, nxt_b, ref, ref1, text;
        this.types.validate.dbr_shape_text_cfg((cfg = {...this.constructor.C.defaults.dbr_shape_text_cfg, ...cfg}));
        ({fontnick, text} = cfg);
        font_idx = this._font_idx_from_fontnick(fontnick);
        R = JSON.parse(this.RBW.shape_text({
          format: 'json',
          text,
          font_idx // formats: json, rusty, short
        }));
        ...
        for (idx = i = 0, len = R.length; i < len; idx = ++i) {
          ...
          d.text = bytes.slice(d.b, nxt_b).toString();
      ...
      * insert_and_walk_outlines(cfg) {
        var bbox, cgid_map, chrs, cid, cids, error, fontnick, gid, glyph, insert_outline, pd, pd_blob, x, x1, y, y1, z;
        ...
        try {
          ...
          for (z of cgid_map) {
            ...
            yield this.db.first_row(insert_outline, {fontnick, gid, cid, glyph, x, y, x1, y1, pd_blob});
          }
          return null;
        } catch (error1) {
          error = error1;
      ...
      //-----------------------------------------------------------------------------------------------------------
      insert_outlines(cfg) {
        return [...(this.insert_and_walk_outlines(cfg))];
      }
