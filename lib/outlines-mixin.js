  ...
  this.Drb_outlines = (clasz = Object) => {
    return class extends clasz {
      ...
      * insert_and_walk_outlines(cfg) {
        var ads, bbox, cgid_map, chrs, error, fontnick, gd_blob, gid, insert_outline, missing, olt, pd, row/* Glyf Data Blob */, specials, x, x1, y, y1, z;
        ...
        try {
          ...
          for (z of cgid_map) {
            ...
            ({x, y, x1, y1} = bbox);
            gd_blob = this._zip(pd);
            row = this.db.first_row(insert_outline, {fontnick, gid, chrs, x, y, x1, y1, olt, gd_blob});
            yield row;
          }
        } catch (error1) {
          error = error1;
