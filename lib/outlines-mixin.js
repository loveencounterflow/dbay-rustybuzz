(function() {
  ...
  this.Drb_outlines = (clasz = Object) => {
    return class extends clasz {
      ...
      shape_text(cfg) {
        var R, ads, bytes, ced_x, ced_y, d, ed_x, font_idx, fontnick, i, idx, len, missing, nxt_b, ref, ref1, special_chrs, text, width;
...
where ( gid != 0 ) and ( sid in ${this.db.sql.V(required_sids)} );`);
      ...
      //   return R
        ...
        //-----------------------------------------------------------------------------------------------------------
      distribute(cfg) {
        var R, ad, adi, ads, brp, brpi, brps, corrected_x, delta_widths, i, j, last_adi, last_brpi, left_adi, left_brpi, len, len1, line, line_ads, lines, mm_p_u, right_adi, right_brpi, width_mm, width_u;
        ...
        for (j = 0, len1 = lines.length; j < len1; j++) {
          ...
          line_ads = ads.slice(line.left_adi, +line.right_adi + 1 || 9e9);
          urge('^5059^', ((function() {
            var k, len2, results;
            results = [];
            for (k = 0, len2 = line_ads.length; k < len2; k++) {
              ad = line_ads[k];
