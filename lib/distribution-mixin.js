  ...
  this.Drb_distribution = (clasz = Object) => {
    return class extends clasz {
      ...
      _distribute_with_db(cfg) {
        var R, ads, create_udfs, insert_into_ads, lines, nxt_brp, prefix, prv_brp, schema, text, v, vnr_1, vnr_1_blob, vnr_2, vnr_2_blob;
        ({ads} = cfg);
...
create view ${schema}.brps as
  ...
  where br is not null;`);
        ...
        console.table(this.db.all_rows(SQL`select vnr, gid, b, x, y, dx, dy, x1, chrs, sid, br from ${schema}.brps order by vnr_blob;`));
        console.table(this.db.all_rows(SQL`select vnr, gid, b, x, y, dx, dy, x1, chrs, sid, br from ${schema}.brps order by abs( deviation ) limit 5;`));
        //.......................................................................................................
        v.dx0 = 0;
        nxt_brp = this.db.single_row(SQL`select vnr, gid, b, x, y, dx, dy, x1, chrs, sid, br from ${schema}.brps where br = 'start' limit 1;`);
        ...
        while (true) {
          ...
          prv_brp = nxt_brp;
          nxt_brp = this.db.single_row(SQL`select vnr, gid, b, x, y, dx, dy, x1, chrs, sid, br from ${schema}.brps order by abs( deviation ) limit 1;`);
          nxt_brp.vnr = jp(nxt_brp.vnr);
          debug('^59686^', prv_brp.vnr, this.hollerith.advance(prv_brp.vnr), nxt_brp.vnr);
          vnr_1 = this.hollerith.advance(prv_brp.vnr);
          vnr_2 = nxt_brp.vnr;
