(function() {
  'use strict';
  var CND, E, SQL, badge, debug, echo, guy, help, info, jr, rpr, urge, warn, whisper;
  ...
  this.Drb_distribution = (clasz = Object) => {
    return class extends clasz {
      ...
      _distribute_with_db(cfg) {
        var adi_1, adi_2, ads, create_udfs, insert_into_ads, line, nxt_brp, prefix, prv_brp, schema, v;
        ({ads} = cfg);
...
create view ${schema}.brps as
  ...
  where br is not null;`);
        ...
        console.table(this.db.all_rows(SQL`select vnr, adi, gid, b, x, y, dx, dy, x1, chrs, sid, br from ${schema}.ads order by adi;`));
        console.table(this.db.all_rows(SQL`select vnr, adi, gid, b, x, y, dx, dy, x1, chrs, sid, br from ${schema}.brps order by adi;`));
        console.table(this.db.all_rows(SQL`select vnr, adi, gid, b, x, y, dx, dy, x1, chrs, sid, br from ${schema}.brps order by abs( deviation ) limit 5;`));
        //.......................................................................................................
        v.dx0 = 0;
        nxt_brp = this.db.single_row(SQL`select vnr, adi, gid, b, x, y, dx, dy, x1, chrs, sid, br from ${schema}.brps where br = 'start' limit 1;`);
        prv_brp = null;
        while (true) {
          if (nxt_brp.br === 'end') {
            break;
          ...
          prv_brp = nxt_brp;
          nxt_brp = this.db.single_row(SQL`select vnr, adi, gid, b, x, y, dx, dy, x1, chrs, sid, br from ${schema}.brps order by abs( deviation ) limit 1;`);
          adi_1 = prv_brp.adi + 1;
          adi_2 = nxt_brp.adi;
          line = this.db.single_value(SQL`select group_concat( chrs, '' ) as chrs from ads where adi between $adi_1 and $adi_2;`, {adi_1, adi_2});
          if (nxt_brp.br === 'shy') {
            line += '-';
          }
          info('^33209^', line);
